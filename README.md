...
| <a name="input_management"></a> [management](#input\_management) | Information required by Station when var.groups and var.applications is used. <br/><br/>  When var.applications or var.groups is set, var.management must be provided. The reason for this is that we do not wish to create two managed resources to dynamically fetch Object ID values for the Graph API Enterprise Application. For cost reasons in Terraform Cloud; as it would require 2 managed resources. With many landing zones it quickly adds up. | <pre>object({<br/>    msgraph_azuread_service_principal_object_id = string<br/>  })</pre> | `null` | no |
