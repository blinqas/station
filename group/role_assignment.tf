locals {
  role_assignments_normalized = {
    for k, v in var.role_assignments : k => merge(v, {
      scope = v.scope == null ? "/subscriptions/${var.subscription_id}" : v.scope
    })
  }

  role_assignments_standard = {
    for k, v in local.role_assignments_normalized : k => v if try(v.pim, null) == null
  }

  role_assignments_pim_eligible = {
    for k, v in local.role_assignments_normalized : k => v if try(v.pim, null) != null && try(v.pim.member_type, "Eligible") == "Eligible"
  }

  role_assignments_pim_active = {
    for k, v in local.role_assignments_normalized : k => v if try(v.pim, null) != null && try(v.pim.member_type, "Eligible") == "Active"
  }
}
